...
async function main() {
  ...
  const CSV_FILENAME = 'Digital Signage - Info - 2025 05 18.csv';
  let csvRaw;
  try {
    // En mode pkg, lit depuis l'asset embarqué
    if (isPkg) {
      csvRaw = fs.readFileSync(path.join(path.dirname(process.execPath), CSV_FILENAME), { encoding: 'latin1' });
    } else {
      csvRaw = fs.readFileSync(path.join(__dirname, CSV_FILENAME), { encoding: 'latin1' });
    }
  } catch (e) {
    // Fallback: tente de lire depuis le chemin pkg.path (utile pour certains contextes)
    csvRaw = fs.readFileSync(path.join(isPkg ? process.pkg.path : __dirname, CSV_FILENAME), { encoding: 'latin1' });
  }
  const csvRows = Papa.parse(csvRaw, { delimiter: ';', skipEmptyLines: true }).data;
  ...
  // Génération du HTML dans un fichier temporaire
  const fsPromises = require('fs').promises;
  const tempHtmlPath = path.resolve('synthese_temp.html');
  await fsPromises.writeFile(tempHtmlPath, html, {encoding: 'utf8'});

  // Ouvre automatiquement synthese_temp.html
  const open = require('child_process').exec;
  if (process.platform === 'win32') {
    open(`start "" "${tempHtmlPath}"`);
  } else if (process.platform === 'darwin') {
    open(`open "${tempHtmlPath}"`);
  } else {
    open(`xdg-open "${tempHtmlPath}"`);
  }

  // Supprime le fichier temporaire après 10 secondes
  setTimeout(() => {
